...
# Porcentajes Ganador
df_nba['FG_PCT_Ganador'] = np.where(df_nba['HOME_TEAM_WINS'] == 1, df_nba['FG_PCT_home'], df_nba['FG_PCT_away'])

# Porcentajes Perdedor
df_nba['FG_PCT_Perdedor'] = np.where(df_nba['HOME_TEAM_WINS'] == 0, df_nba['FG_PCT_home'], df_nba['FG_PCT_away'])
df_nba['FG3_PCT_Perdedor'] = np.where(df_nba['HOME_TEAM_WINS'] == 0, df_nba['FG3_PCT_home'], df_nba['FG3_PCT_away'])
df_nba['FT_PCT_Perdedor'] = np.where(df_nba['HOME_TEAM_WINS'] == 0, df_nba['FT_PCT_home'], df_nba['FT_PCT_away'])

# Totales globales
df_nba['puntos_total'] = df_nba['PTS_home'] + df_nba['PTS_away']
df_nba['ast_total'] = df_nba['AST_home'] + df_nba['AST_away']
df_nba['reb_total'] = df_nba['REB_home'] + df_nba['REB_away']


# ==========================================
#paginas para explora
# ==========================================
...
def page_reb(df_filtrada):
    ...
    st.markdown("Análisis de los rebotes y puntos anotados")

    avg_ganador = df_filtrada['REB_Ganador'].mean()
    avg_perdedor = df_filtrada['REB_Perdedor'].mean()
    
    total_partidos = len(df_filtrada)
...
def page_jueg(df_filtrada):
    ...
    pcts = df_filtrada['Tipo_Juego'].value_counts(normalize=True) * 100
    ...
    fig.update_layout(
        ...
        plot_bgcolor='rgba(0, 0, 0, 0)',
        xaxis=dict(showgrid=False, zerolinecolor='#444444'), 
...
def page_tl(df_filtrada):
    st.subheader("🏀 Explorador de Datos NBA")

    st.dataframe(
        df_filtrada, 
        use_container_width=True, 
        column_config={
            "SEASON": st.column_config.NumberColumn("Temporada", format="%d"),
            "Margen_Victoria": st.column_config.NumberColumn(
                "Margen (PTS)",
                format="%d 🏀"
            )
        },
...
def main():
    ...
    pages = {
        ...
        "🏀 Explorador de Datos NBA": page_tl
    }
    ...
    if Tipo_seleccionado:
        df_filtrado = df_nba[df_nba['Tipo_Juego'].isin(Tipo_seleccionado)]
    else:
        df_filtrado = df_nba
      
    # Envío del dataframe filtrado a la página correspondiente
